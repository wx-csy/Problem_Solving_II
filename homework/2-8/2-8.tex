...
\begin{document}
  ...
  \begin{codebox}
      ...
      \li $\attrib{y}{next} \gets \const{nil}$
          \End
  \end{codebox}
  Operation \proc{Delete} can be implemented in a best-case running time of $O(1)$, as long as we use a sentinel (a dummy object), as the textbook states in page 238-239.
  \begin{codebox}
      \Procname{$\proc{Delete'}(L, x)$}
