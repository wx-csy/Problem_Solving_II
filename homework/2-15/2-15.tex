...
\begin{document}
  ...
  This statement is true. Note that $d(i, j) = 1$ if and only if $i$ and $j$ are adjacent. Therefore, for every edge $uv \in V(G)$, $d_G(u, v) = 1$, thus $d_H(\phi(u),\phi(v)) = 1$, i.e $\phi(u)\phi(v)\in V(H)$. Likewise, if $\phi(u)\phi(v)\in V(H)$, then $uv \in V(G)$. By the definition of isomorphism, $H$ and $G$ are isomorphic.
\end{document}
